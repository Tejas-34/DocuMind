<template>
  <div class="py-8 px-4 sm:px-8 max-w-6xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2">
      ...
      <button
        type="button"
        @click="triggerUploadBrowse"
        class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-[#153826] hover:bg-[#1b4932] text-white rounded-xl text-xs font-semibold shadow-xs transition-all active:scale-[0.99] shrink-0 cursor-pointer"
      >
        <Plus class="w-4 h-4" />
    ...
    <div
      ...
      class="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-800 text-rose-600 dark:text-rose-400 text-xs font-medium flex items-center gap-2"
    >
    ...
    >
      <div v-if="selectedDoc" class="space-y-3.5 font-sans text-xs">
        ...
        <div>
          ...
          <span class="font-mono bg-gray-100 dark:bg-gray-800 p-2 rounded-xl block text-[11px] select-all border border-gray-200/60 dark:border-gray-700/60">
            {{ selectedDoc.id }}
          </span>
        </div>
        <div class="grid grid-cols-2 gap-3 pt-1">
          <div class="p-3 rounded-2xl bg-gray-50/80 dark:bg-[#16221c]/40 border border-gray-100 dark:border-gray-800">
